...
public class SearchController {
	...
	public ModelAndView searchByTitleCat(String cat_id, String bookTitle, Integer PAGE, Long BOOKID, String action,
			String action1, HttpSession session, HttpServletRequest request) {
		...
		if (cat_id == null || cat_id.trim().isEmpty()) {
			...
			if (loginUser != null) {
				...
				for (Book book1 : insertBookList) {
					jjim.setUser_id(loginUser);
					jjim.setIsbn(book1.getIsbn());

					// 찜 상태 체크
					boolean isLiked = jjimservice.isBookLiked(jjim) > 0;
					book1.setLiked(isLiked);

					// 찜한 사람 수 계산 (예: 찜한 사람 수를 가져오는 메소드 호출)
					int likeCount = jjimservice.getLikeCount(book1.getIsbn());
					book1.setLikecount(likeCount);
				}
			}
			...
			sek.setEnd(end);
			sek.setBook_title(bookTitle);
			sek.setCat_id(cat_id);

			// cat_id에 따라 다르게 필터 검색을 적용한다.
			Category cat = this.categoryService.getCategoryDetail(cat_id);
			if (cat.getParent_id() == null) {
				String key = "top";
				System.out.println("key : " + key);
				sek.setKey(key);
			} else if (cat.getParent_id().equals("0") || cat.getParent_id().equals("1")) {
				String key = "mid";
				System.out.println("key : " + key);
				sek.setKey(key);
			} else {
	...
	public ModelAndView goDetailSearch(HttpServletRequest request) {
	...
	public ModelAndView detailSearch(String TITLE, String AUTHOR, String PUBLISHER, String PUB_DATE_START,
		...
		ds.setBook_title(TITLE);
		ds.setAuthors(AUTHOR);
