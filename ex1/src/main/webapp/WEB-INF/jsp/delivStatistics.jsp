...
<body>
	<div class="nav">
		<a href="/adminPage">HOME</a> <a href="/manageGoods">상품 관리</a> <a
			href="/anslist">고객 문의</a> <a href="/adminevent">이벤트 관리</a> <a
			href="/adminrer">교환 및 반품 현황</a> <a href="/goStatistics">통계 내역</a> <a
			href="/categories">필터 관리</a>
		<!--         관리자 grade==9만들동안 이용 -->
	...
	<div class="content-container" align="center">
		<div class="navigation-box">
			<a href="/goStatistics">판매 통계</a> <a href="/userStatistics">회원 통계</a>
		...
		<div class="table-container">
			...
			<form action="/orderStatistics" method="get">
				<table border="1">
					<tr>
						<td colspan="7" align="right"><select id="SELECT"
							name="SELECT">
								<option value="" ${SELECT == '' ? 'selected' : ''}>전체보기</option>
								<option value="배송 준비중" ${SELECT == '배송 준비중' ? 'selected' : ''}>배송
									준비중</option>
								<option value="배송 중" ${SELECT == '배송 중' ? 'selected' : ''}>배송
									중</option>
								<option value="배송 취소" ${SELECT == '배송 취소' ? 'selected' : ''}>배송
									취소</option>
								<option value="배송 완료" ${SELECT == '배송 완료' ? 'selected' : ''}>배송
									완료</option>
							...
							<td>
								<form action="/orderStatistics" method="get">
									...
									<select name="deliveryStatus" onchange="this.form.submit()">
										<option value="0"
											${list.delivery_status == 0 ? 'selected' : ''}>배송
											준비중</option>
										<option value="1"
											${list.delivery_status == 1 ? 'selected' : ''}>배송 중</option>
										<option value="2"
											${list.delivery_status == 2 ? 'selected' : ''}>배송 취소</option>
										<option value="3"
											${list.delivery_status == 3 ? 'selected' : ''}>배송 완료</option>
									</select>
								</form>
							</td>
						</tr>
